...
- **AI improv quips**: octopus chats are occasionally generated by haiku from the session's real work log (✨ toggle; throttled to ~1/min; the generator session's hooks are auto-ignored so no ghost rooms appear) — `server/quips.mjs`. Facility encounters also spark small talk (two octopuses meeting at the coffee machine / water cooler).
